...
pivot_ef = ldf.pivot(
    index="entry_point_dist_bin", columns="revisit_bin", values="ef"
...
if cdf is not None:
    ...
    ax_pv.legend(
        fontsize=9, facecolor="#222233", labelcolor="white", edgecolor="#555566"
    )
    ...
    ax_rat.axvline(
        1.0, color="#ff6b6b", lw=2, linestyle="--", label="ratio=1 (perfect)"
    )
    ax_rat.axvline(
        cdf["ratio"].median(),
        color="#ffd166",
    ...
    rows2 = [
        ...
        (
            "Under-predictions",
            f"{n_under}  ({100 * n_under / len(cdf):.1f}%)",
            "#ff6b6b",
        ),
