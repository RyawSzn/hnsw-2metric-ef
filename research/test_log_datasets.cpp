...
ProbeRes probe_query_log(HierarchicalNSW<float>* alg_hnsw, const float* query, int L, int ef_probe_cap) {
    ...
    float sum_log_tot = 0, sum_log_vis = 0;
    ...
    while (!candidate_set.empty()) {
        ...
        for (int j = 0; j < sz; j++) {
            ...
            float w_log = 0.0f;
            if (lowerBound > 0) {
                w_log = std::max(0.0f, std::log(lowerBound / std::max(d, 1e-6f)));
            }
            sum_log_tot += w_log;
            if (is_revisit) sum_log_vis += w_log;
    ...
    ProbeRes res;
    res.rv_log = sum_log_vis / std::max(1e-6f, sum_log_tot);
    res.m_lid = m_q;
    return res;
}
...
int main(int argc, char** argv) {
    ...
    // Use 1000 for text, 200 for others to keep it fast
    int nq = (dataset.find("glove") != std::string::npos) ? 1000 : 250;
    ...
    for (int i = 0; i < nq; i++) {
        ...
        pr[i] = probe_query_log(alg_hnsw, q.data(), alg_hnsw->maxlevel_, 100);
    }
    ...
    std::string out_path = "research/log_sweep_" + dataset + ".csv";
    std::ofstream out(out_path);
    out << "query_idx,ef_true,RV_log,m_LID\n";

    for (int i = 0; i < nq; i++) {
        out << q_idx[i] << "," << ef_true[i] << "," << pr[i].rv_log << "," << pr[i].m_lid << "\n";
    }
    out.close();
