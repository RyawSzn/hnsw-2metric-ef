...
def main():
    ...
    df = pd.read_csv('deep_score_recall.csv')
    
    print("Loading deep-image distances from HDF5 to compute LID...")
    hdf5_path = '/home/ryawszn/experiments/2metric/data/deep-image-96-angular.hdf5'
    with h5py.File(hdf5_path, 'r') as f:
        distances = np.array(f['distances'])
    
    num_queries = distances.shape[0]
    k_lid = 100 # Compute LID based on 100 nearest neighbors
    
    print("Computing LIDs...")
    lids = compute_lid_from_dists(distances, k_lid, num_queries)
    df['LID'] = lids
    
    # Drop rows where LID, score or recall is NaN
